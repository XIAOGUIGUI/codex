...
const USER_INPUT_REQUEST_INPUT_FIXTURE: &str = "user-input-request.command.input.schema.json";
const USER_INPUT_REQUEST_OUTPUT_FIXTURE: &str = "user-input-request.command.output.schema.json";
...
#[derive(Debug, Clone, Serialize, JsonSchema)]
#[serde(deny_unknown_fields)]
#[schemars(rename = "user-input-request.command.input")]
pub(crate) struct UserInputRequestCommandInput {
    pub session_id: String,
    pub turn_id: String,
    pub cwd: String,
    pub transcript_path: NullableString,
    #[schemars(schema_with = "user_input_request_hook_event_name_schema")]
    pub hook_event_name: String,
    pub tool_name: String,
    pub call_id: String,
...
#[serde(deny_unknown_fields)]
#[schemars(rename = "user-input-request.command.output")]
pub(crate) struct UserInputRequestCommandOutputWire {
...
pub fn write_schema_fixtures(schema_root: &Path) -> anyhow::Result<()> {
    ...
    write_schema(
        &generated_dir.join(USER_INPUT_REQUEST_INPUT_FIXTURE),
        schema_json::<UserInputRequestCommandInput>()?,
    )?;
    write_schema(
        &generated_dir.join(USER_INPUT_REQUEST_OUTPUT_FIXTURE),
        schema_json::<UserInputRequestCommandOutputWire>()?,
    )?;
    write_schema(
...
fn user_input_request_hook_event_name_schema(_gen: &mut SchemaGenerator) -> Schema {
    string_const_schema("UserInputRequest")
}
...
mod tests {
    ...
    use super::USER_INPUT_REQUEST_INPUT_FIXTURE;
    use super::USER_INPUT_REQUEST_OUTPUT_FIXTURE;
